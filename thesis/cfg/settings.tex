...
\setlength{\cftbeforesecskip}{0.4em plus 0.1em}
\setlength{\cftbeforesubsecskip}{0.25em plus 0.05em}
...
% Ширина колонки номера: «номер + один пробел». В TNR 14pt цифра ≈0.5em,
% точка ≈0.3em. «1 » ≈ 0.8em, «1.1 » ≈ 1.6em, «1.1.1 » ≈ 2.4em — берём с запасом.
\setlength{\cftsecnumwidth}{1.1em}
\setlength{\cftsubsecnumwidth}{1.9em}
...
% Гасим растягивание пробелов внутри многострочных пунктов TOC.
% Стандартный \@dottedtocline делает строки justified, что портит вид
% при длинных заголовках. Делаем правое поле упругим (plus 1fil), чтобы
% последнее слово на строке не дотягивалось до правого края title-колонки.
\makeatletter
\renewcommand*\@dottedtocline[5]{%
	\ifnum #1>\c@tocdepth \else
		\vskip \z@ \@plus.2\p@
		{\leftskip #2\relax \rightskip \@tocrmarg plus 1fil
			\parfillskip -\@tocrmarg
			\parindent #2\relax\@afterindenttrue
			\interlinepenalty\@M
			\hyphenpenalty=10000     % запрет автопереносов в TOC (п.4.4.4 ГОСТ)
			\exhyphenpenalty=10000   % и по существующим дефисам компаунд-слов
			\leavevmode
			\@tempdima #3\relax
