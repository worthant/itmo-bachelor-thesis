...
- [x] Оценить возможность замены float FFT на esp-dsp `dsps_fft2r_fc32`
  - note: используем
- [ ] Оптимизировать mfcc.c (PIE для циклов и операций sqrtf)
- [ ] Оценить возможность&надобность квантования мел-матрицы (float->int16/8)
...
- [x] Scatter plot: accuracy vs model size
  - [x] note: такой себе получился артефакт имхо, не особо репрезентативный, просто две точки
...
- [ ] ? Оптимизация производительности (PIE/dsp/float->int/e.t.c.)
